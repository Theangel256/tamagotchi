...
from src.gui.components import (
    CharacterDisplay, StatsPanel, ActionPanel, InventoryPanel, GamePanel
)
...
class TamagotchiApp:
    ...
    def _setup_notebook(self):
        ...
        self.tab_game = tk.Frame(self.notebook, bg="#f7f7f7")
        ...
        self.game_panel = GamePanel(self.tab_game, self.tama, self.game, self._on_event)
        self.game_panel.pack(fill="both", expand=True, padx=8, pady=8)

    def _setup_status_area(self):
        # Area inferior para mensajes / eventos
        self.status_frame = tk.Frame(self.root, bg="#ededed", height=48)
        self.status_frame.pack(fill="x", side="bottom")
        self.event_label = tk.Label(self.status_frame, text="Bienvenido!", bg="#ededed")
        self.event_label.pack(padx=8, pady=8, anchor="w")

    def _on_event(self, mensaje: str):
        # Callback desde componentes; muestra mensaje y actualiza UI
        self.event_label.config(text=mensaje)
        self.update_ui()
